...
class PiVideoStream:
    ...
    def update(self):
        ...
        for f in self.stream:
            ...
            # if the thread lock variable is set stop the thread
            if self.stopped:
                self.stream.close()
            ...
            # clear the stream in preperation for the next one
    ...
    def pub_image(self):
        ...
        if rospy.is_shutdown():
            ...
            # Create image object
            msg = Image()
            msg.header.frame_id = 'bgr_cam_raw'
...
# this Class can run standalone or can be called from another script.
if __name__ == '__main__':

    vs = PiVideoStream(framerate=20, ROS=True).start() # start picamera using defaults with ROS node

    while not rospy.is_shutdown(): 
        time.sleep(1) ## time to sleep so we don't waste CPU power
    
    vs.stop() # if ROS shuts done kill the camera thread
